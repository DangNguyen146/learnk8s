...
mute_alert_service=docker_cpu_config.mute_alert_service
...
def get_query_sevice_group_host(name):
    filter="cpu_usage:>80 AND Name.keyword:\"%s\"" % name
    myquery = {
    "size": 0,
    "query": {
    ...
    try:
        ...
        if rs['count'] > 4:
            ...
            for bucket in buckets:
                ...
                if service_name in mute_alert_service :
                    print("Skip alert for service %s" % service_name)
                    continue
                ...
                if count >= 3:
                    ...
                    # print(key)
                    redis_key = "alert_docker_stats_cpu_" + key
                    status = check_alert_threshold(redis_key)
                    if status:
                        if not redis_key_exists(redis_key):
                            title = "PROBLEM CPU usage service: %s %s" % (key, alert_level)
                            details = "There are more than %s events in service %s" % (str(count), key)
                            kibana = kibana_index.replace("webservices-v2",key)
                            kibana = kibana.replace("vStaging-Rancher02",host)
